...
@interface HeFeedBackReplyVC ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate>
...
- (void)initView
{
    ...
    NSInteger rate = [lastestReplyDict[@"rate"] integerValue];
    if (rate == -2 || rate == 1 || rate == 2) {
...
- (void)loadReplyContent
{
    ...
    [self showHudInView:self.tableview hint:@"加載中..."];
    ...
    [AFHttpTool requestWihtMethod:RequestMethodTypePost url:requestUrl params:params success:^(AFHTTPRequestOperation* operation,id response){
        ...
        if ([resultDict isKindOfClass:[NSDictionary class]]) {
            ...
            if (error) {
                ...
                if ([allkey count] == 0) {
                    errorString = [[NSMutableString alloc] initWithString:@"評價失敗"];
                }
                [self showHint:errorString];
                return;
            }
